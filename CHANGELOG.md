...
🔗 Compare:
https://github.com/devalltect00/Custy/compare/v2.1.1...HEAD
...
since v2.1.1 into the next patch release.

### Readme Presentation

#### Project

- Add the Custy CLI preview near the top of the README.
- Store the preview locally so it renders independently on GitHub and GitLab.
- Preserve the existing badges, project overview, workflow guidance, and usage
...
- Add regression coverage for the shared ignore and quality-tool contracts.
...
- Run the focused regression test and repository quality checks.

### 🧠 Improvements

#### Readme

- Add concise metadata for Custy v2.1.1, Python compatibility, supported
- Align the repository metadata helper heading with the established section
- Add the project contact alongside the existing Devalltect maintainer footer.
- Preserve the existing installation, CLI, dry-run, Docker, Make, security,

### 📚 Documentation

- Refine README release metadata and project presentation.
...
- Preserve the public CLI, configuration, dry-run guarantees, and Python 3.14+
...
🔗 Compare:
https://github.com/devalltect00/Custy/compare/v2.1.0...v2.1.1
...
Correct version reporting and make the daily development profile honor its
documented commit-and-push boundary.

### Fixed

#### Workflow

- Replace setuptools-scm's post-release scheme with development-version
- Keep the source fallback version synchronized at `2.1.1`.
- Remove version preparation, workflow initialization, release-artifact
- Open only the commit-message file during the Dev profile.
- Push branch `HEAD` without attempting to push a resolved-but-uncreated tag.
- Preserve the existing release and full-profile tag behavior through explicit

### 📚 Documentation

#### Workflow

- Add profile, step, editor, push-orchestration, and remote-push regression
- Update the English and Indonesian documentation baseline and Dev workflow

**Tags**

release • workflow • docs • stable
...
- Keep GitHub Actions and GitLab CI validation aligned with Python 3.14, the
- Keep generated Python bytecode ignored and untracked even where active backup
- Make CLI help regression tests stable across ANSI-capable runners and
...
release • docs • tests • ci • stable • changelog • repository-links • git-remotes • credentials • github • gitlab • github-actions • gitlab-ci • push • pipeline • versioning • commitizen • git-hooks • pre-commit • release-messages • editor • terminal-ui • docker • ghcr • dry-run • documentation
