# app/core/git_ops/engine/git_workflow_engine.deprecated.py
"""
GitCommitTagger (Refactored)
...
NO orchestration logic here anymore.
"""
...
from app.core.git_ops.helper import (
    CommitizenHelper, GitHelper,
    classify_commit_type, get_last_tag_before,
...
from app.core.changelog.old import ChangelogGenerator
...
import app.core.pipeline.steps
...
class GitWorkflowEngine:
    """
    Lightweight entrypoint for pipeline execution.
    ...
    def __init__(
        ...
        self.message_path: Path | None = Path(message_file) if message_file else None
        self.tag_input: str | None = tag
        self.tag_msg_input: str | None = tag_msg
        self.tag_msg_file: Path | None = Path(tag_msg_file) if tag_msg_file else None
        ...
        self.git_service = create_git_service(dry_run=dry_run)
        ...
        self.workflow_manager = BranchWorkflowManager(
    ...
    def _set_silent_mode(self):
        """
        Enable silent mode for all runner-based components.
        """

        # GitService → executor → runner
        if hasattr(self, "git_service"):
            self.git_service.executor.runner.set_silent(True)

        # Legacy GitHelper (temporary)
        if hasattr(self, "git"):
            self.git.runner.set_silent(True)
    ...
    def execute_all(self) -> None:
        """
        Execute full pipeline using dynamic builder.
        ...
        #     ValidateStep(),
        #     WorkflowInitStep(),
        #     EditCommitMessageStep(),
        #     ChangelogStep(),
        #     CommitStep(),
        #     TagStep(),
        #     BackupStep(),
        #     PushStep(),
        #     CleanupStep(cleanup_backups=True),
        ...
        pipeline = builder.build(pipeline_config)
    ...
    def run_commands(self, commands: list[str]) -> None:
        """
        Execute pipeline based on CLI commands.

        Examples:
            run_commands(["commit", "tag", "push"])
            run_commands(["release"])
            run_commands(["release+backup"])
        """
    ...
    def execute_commit(self) -> None:
        ...
        self.backup_commit_message()
        print(f"\n{Fore.GREEN}[*] Commit changes operation...{Style.RESET_ALL}\n")
    ...
    def execute_tag(self):
        ...
        print(f"\n{Fore.LIGHTYELLOW_EX}[*] Backup phase...{Style.RESET_ALL}\n")
        self.backup_tag_message_with_check()
        print(f"\n{Fore.GREEN}[*] tag and bump operation...{Style.RESET_ALL}\n")
        self._tag()
    ...
    def ensure_git_repo(self):
        if not self.git.is_get_repo():
            ...
            logger.warning("💡 Hint: Initialize git repository using `git init`")
            # sys.exit(1)
            raise ValidationError(
                message=f"Git repository not found.",
                hint="Hint: Initialize git repository using `git init`")
    def ensure_version_file(self) -> None:
        if not self.version_file.exists():
            logger.error(f"❌ [red]ERROR[/red]: Message file '{self.version_file}' not found.")
            raise ValidationError(
                message=f"Message file '{self.version_file}' not found.",
                hint="Hint: Run `custy init config` to generate it.")
    def ensure_commit_message_file(self) -> None:
        if not self.message_path.exists():
            logger.error(f"❌ [red]ERROR[/red]: Commit message file '{self.message_path}' not found.")
            raise ValidationError(
                message=f"Commit message file '{self.message_path}' not found.",
                hint="Hint: Run `custy init config` to generate it.")
    def ensure_tag_message_file(self) -> None:
        if not self.tag_msg_file.exists():
            logger.error(f"❌ [red]ERROR[/red]: Tag message file '{self.tag_msg_file}' not found.")
            raise ValidationError(
                message=f"Tag message file '{self.tag_msg_file}' not found.",
                hint="Hint: Run `custy init config` to generate it.")
    def ensure_staged_changes(self) -> None:
        from app.core.git_ops.helper.git_helper import GitService, CommandResult

        class FakeExecutor:
            def diff_cached_quiet(self):
                return CommandResult(returncode=1)
        service = GitService(FakeExecutor())
        service.has_staged_files()
        ...
        if self.auto_stage:
            logger.warning("⚠️ No staged changes detected.")
            logger.info("📦 Auto-running: git add .")
            ...
            if not self.dry_run:
                ...
                if not self.git.has_staged_files():
                    if self.force_commit:
                        ...
                        return
                    logger.warning("⚠️ [yellow]Warning[/yellow]: Still no staged changes after `git add .`")
                    confirm = input(f"{Fore.LIGHTYELLOW_EX}Are you sure you want to continue? (y/n): {Style.RESET_ALL}")
                    confirm = confirm.strip().lower()
                    if confirm == "y" or confirm == "yes":
                        logger.warning("⚠️ Continuing despite no staged. files.")
                        return
                    else:
                        logger.warning("🛑 Aborting due to empty staging.")
                        sys.exit(1)


            staged_files = self.git.list_staged_files()
            if staged_files:
                logger.info(colored("📝 File staged for commit:", "cyan"))
                for file in staged_files:
                    logger.info(colored(f"  -{file}", "green"))
            else:
                logger.warning("⚠️ No files were staged.")
                if self.force_commit:
                    logger.warning("⚠️ Proceeding with empty due to --force-commit.")
                    return
                self._error_exit("No files staged after auto-stage.")
        elif self.force_commit:
            logger.warning("⚠️ No staged files, but proceeding due to --force-commit.")
            return
        else:
            logger.error("❌ Error: No staged changes to commit.")
            logger.warning("💡 Hint: Stage files using `git add <file>` or `git add .`")
            # sys.exit(1)
            raise ValidationError(
                message=f"No staged changes to commit.",
                hint="Hint: Stage files using `git add <file>` or `git add .`")
    #####/
    ##########/
    ...
    def _resolve_tag_message(self) -> None:
        """
        Resolve tag message: use CLI --tag-msg, or fallback to tag-message.txt or tag name.
        ...
        if self.tag_msg_input:
            ...
            logger.info("✅ Using tag message from --tag-msg CLI input.")
            return

        tag_msg_path = Path(self.tag_msg_file or CUSTY_TAG_MESSAGE_TEMPLATE)
        ...
        if not tag_msg_path.exists():
            ...
            logger.info(f"📝 Created default tag message file: {tag_msg_path}")

    def backup_tag_message_with_check(self):
        # Read edited content
        if self.tag_msg_file.exists():
            self.tag_msg = self.tag_msg_file.read_text(encoding="utf-8").strip()
            logger.info(f"📄 Using tag message from: {self.tag_msg_file}")
            self.backup_tag_message()
        else:
    ...
    def _update_version_file(self) -> None:
        ...
        content = f'# {self.version_file}\n\n__version__ = "{self.tag}"\n'

        if self.dry_run:
            logger.info(f"(dry-run) Would write to {self.version_file}: {content.strip()}")
            # print(f"(dry-run) Would generate tag: {self.tag}")
        else:
            if not self.version_file.exists():
                logger.error(f"❌ [red]ERROR[/red]: Version file '{self.version_file}' not found.")
                sys.exit(1)
            self.version_file.write_text(content)
            # logger.info(f"✅ Updated version file: {self.version_file}")
            logger.info(f"✅ [dim]{self.version_file}[/dim] [green]Updated[/green]. Version updated to {self.tag}")
    ...
    def _open_editor(self, path: Path) -> None:
        ...
        for editor_cmd in editors:
            try:
                ...
                continue
        logger.error("❌ ERROR: Could not open editor. Please edit the message file manually.")
        sys.exit(1)
        ...
        if commit_type:
            if commit_type not in ALLOWED_COMMIT_TYPES:
                if not self.force_tag:
                    ...
                    print(
                        "💡 Note: Don't forget to manually update the version in commit-message.txt",
                    )
                    print(
            ...
            logger.warning("⚠️ Forcing tag despite unrecognized commit type.")
    ...
    def _push(self, remote: str = "origin") -> None:
        ...
        """
        logger.info(f"🔁 Pushing to {remote} remote...")
        self.git.runner.run(
        ...
        if not getattr(self, "skip_tag", False) and self.tag:
            ...
            logger.info(f"⏭️ Tag push to {remote} remote skipped.")

    def _error_exit(self, message: str) -> None:
        logger.error(f"❌ [red]ERROR[/red]: {message}")
        sys.exit(1)

    def backup_commit_message(self) -> None:
        if not self.message_path.exists():
            return
        ...
        try:
            ...
            logger.info(f"🗂️ Commit message backed up to: {self.backup_commit_message_path}")
            ...
            logger.error(f"⚠️ Failed to backup commit message: {e}")

    def backup_tag_message(self) -> None:
        """
        ...
        try:
            ...
            logger.info(f"🗂️ Tag message backed up to: {self.backup_tag_message_path}")
            old_files = self.backup_manager._prune_old_backups(
                backup_dir=backup_dir,
            ...
            logger.error(f"⚠️ Failed to backup tag commit message: {e}")
    ...
    def _write_template_to_message_file(self, content: str):
        """
        Write prefilled message content to both commit-message.txt and tag-message.txt
        """
    ...
    def _generate_release_notes(self):
        """
        Generate commit-message.txt and tag-message.txt using ReleaseNoteBuilder.
        """
        version = self.tag
