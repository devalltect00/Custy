# app/core/git_ops/helper/git_helper1.deprecated.py
...
class CommandResult:
    def __init__(
        ...
        self.stdout = stdout
        self.stderr = stderr
        self.skipped = skipped  # True if dry-run
    ...
    @classmethod
    def from_completed(cls, result: subprocess.CompletedProcess):
        return cls(
...
class GitCommandExecutor(DryRunSupport):
    ...
    def diff_cached_quiet(self) -> CommandResult:
        ...
        # Handle dry-run (runner returns None)
        if result is None:
...
class GitService:
    ...
    Knows NOTHING about subprocess or dry-run.
    """

    def __init__(self, executor: GitCommandExecutor):
    ...
    def has_staged_files(self) -> bool:
        ...
        # Dry-run behavior (decide your policy here)
        if result.skipped:
            logger.info("⚠️ Dry-run: assuming no staged files.")
            return False
        ...
        logger.info("⚠️ No staged changes found.")
        return False


def create_git_service(dry_run: bool = False) -> GitService:
    executor = GitCommandExecutor(dry_run=dry_run)
