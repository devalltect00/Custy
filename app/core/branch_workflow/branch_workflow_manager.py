...
class BranchWorkflowManager(DryRunSupport):
    """
    ...
    def _ensure_no_staged_changes(self, context: str = "merge") -> None:
        ...
        Dry-run mode still inspects the index so the preview can report the
        safety condition, but it does not terminate because no branch mutation
        will be executed.

        Args:
            context:
                Human-readable operation that requires a clean index.
        ...
        if self.git.has_staged_files():
            ...
            if self.runner.get_is_dry_run():
                print(
                    "🧪 Dry-run: real execution would stop here; "
                    "continuing the simulated workflow."
                )
                return
