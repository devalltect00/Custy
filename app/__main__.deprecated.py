...
from .core.git_ops.git_commit_tagger import GitCommitTagger
...
def handle_changelog(args):
    ...
    tool = build_git_tool(args, with_tagging=True)
    tool.tag = latest_tag
...
def handle_docs(args):
    from .utils.docs_generator import DocsGenerator
...
def main() -> None:
    ...
    add_tagging_arguments(changelog_parser)
    ...
    add_common_arguments(docs_parser)
    add_tagging_arguments(docs_parser)
    add_sync_argument(docs_parser)
    add_force_changelog_argument(docs_parser)
    add_control_debug_argument(docs_parser)
